 ...
 *         Copyright 2019
 *         A class for chansey cycles where a group of Objects are picked from randomly to decide the Object
 */
public class ChanseyCycle extends Cycle {
	...
	// The group of Objects to pick from
	...
	/**
	 * @return a randomly picked Object from this cycle
	 */
	@Override
	public ArrayList<Object> fun() {
		...
		for(int j = 0; j < loops+1; j++) {
			...
			tempObject = choices[indexChoice];
			if(tempObject instanceof ChanseyCycle) {
				output.addAll(((ChanseyCycle)tempObject).fun());
			} else if(tempObject instanceof Cycle) {
				output.addAll(((Cycle)tempObject).fun());
			} else {
